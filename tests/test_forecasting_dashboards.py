...
class ForecastingDashboardTests(unittest.TestCase):
    ...
    def test_render_model_vs_market_dashboard_writes_artifacts_with_optional_evidence(
        self,
    ):
        rows_payload = {
            "rows": [
    ...
    def test_render_model_vs_market_dashboard_sanitizes_markdown_sensitive_fields(self):
        rows_payload = {
            "rows": [
                {
                    "contract_id": "election-yes\n| injected | row |",
            ...
            "rows": [
                {
                    "contract_id": "election-yes\n| injected | row |",
                    "llm_probability": 0.6,
                    "summary": "candidate momentum\n## injected heading",
                    "consistency_notes": ["note-1\n- injected bullet"],
                    "citations": ["memo-1"],
                }
            ]
        }
        ...
        self.assertEqual(stdout.getvalue(), "")
        self.assertIn("# Model vs Market Dashboard", markdown)
        self.assertIn(
            "| election-yes \| injected \| row \| | 0.6200 | 0.5500 | 0.0700 | politics ## injected | election - injected |",
            markdown,
        )
        self.assertIn(
            "- `election-yes \| injected \| row \|` — llm_evidence/ok; note-1 - injected bullet; llm summary: candidate momentum ## injected heading",
            markdown,
        )
        self.assertNotIn("\n## injected heading", markdown)
        self.assertNotIn("\n| injected | row |", markdown)
