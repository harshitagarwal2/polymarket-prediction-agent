...
class ModelVsMarketRow:
    ...
    def from_payload(cls, payload: Mapping[str, object]) -> 'ModelVsMarketRow':
        ...
        model_probability = _coerce_probability(
            payload.get('model_probability', payload.get('fair_value', payload.get('prediction'))),
...
def _load_dashboard_payload(source: str | Path | Sequence[Mapping[str, object]] | Mapping[str, object]) -> object:
...
def load_model_vs_market_rows(
    ...
    return tuple(ModelVsMarketRow.from_payload(row) for row in raw_rows if isinstance(row, Mapping))
...
def build_model_vs_market_dashboard(
    ...
    *,
    ...
    average_model_probability = sum(row.model_probability for row in rows) / contract_count
    average_market_probability = sum(row.market_probability for row in rows) / contract_count
    average_gap = sum(
        row.model_probability - row.market_probability for row in rows
    ) / contract_count
    average_abs_gap = sum(
        abs(row.model_probability - row.market_probability) for row in rows
    ) / contract_count
    top_contract_edges = [
        {
    ...
    fallback_count = sum(
        1 for surface in consistency_surfaces if surface.mode == 'deterministic_fallback'
    )
...
def render_model_vs_market_markdown(dashboard: ModelVsMarketDashboard) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Average model probability: {_format_float(summary['average_model_probability'])}",
            f"- Average market probability: {_format_float(summary['average_market_probability'])}",
            f"- Average model-market edge: {_format_float(summary['average_model_minus_market'])}",
            f"- Average absolute edge: {_format_float(summary['average_abs_model_minus_market'])}",
        ]
    )
    ...
    calibration = summary.get('calibration_comparison') if isinstance(summary, dict) else None
            ...
            [
                ...
                '| Brier score | {model_brier} | {market_brier} | {delta_brier} |'.format(
                    model_brier=_format_float(
                        dashboard.model_score.brier_score if dashboard.model_score else None
                    ),
                    market_brier=_format_float(
                        dashboard.market_score.brier_score if dashboard.market_score else None
                    ),
                    delta_brier=_format_float(
                        calibration.get('model_minus_market_brier')
                        if isinstance(calibration.get('model_minus_market_brier'), (int, float))
                        else None
                    ),
                ),
                '| Log loss | {model_log_loss} | {market_log_loss} | {delta_log_loss} |'.format(
                    model_log_loss=_format_float(
                        dashboard.model_score.log_loss if dashboard.model_score else None
                    ),
                    market_log_loss=_format_float(
                        dashboard.market_score.log_loss if dashboard.market_score else None
                    ),
                    delta_log_loss=_format_float(
                        calibration.get('model_minus_market_log_loss')
                        if isinstance(calibration.get('model_minus_market_log_loss'), (int, float))
                        else None
                    ),
                ),
                '| Accuracy | {model_accuracy} | {market_accuracy} | {delta_accuracy} |'.format(
                    model_accuracy=_format_float(
                        dashboard.model_score.accuracy if dashboard.model_score else None
                    ),
                    market_accuracy=_format_float(
                        dashboard.market_score.accuracy if dashboard.market_score else None
                    ),
                    delta_accuracy=_format_float(
                        calibration.get('model_minus_market_accuracy')
                        if isinstance(calibration.get('model_minus_market_accuracy'), (int, float))
                        else None
                    ),
                ),
                '| ECE | {model_ece} | {market_ece} | {delta_ece} |'.format(
                    ...
                    delta_ece=_format_float(
                        ...
                        if isinstance(calibration.get('model_minus_market_ece'), (int, float))
                        else None
                    ),
                ),
                '',
    ...
    for row in summary['top_contract_edges']:
        lines.append(
    ...
    for surface in dashboard.consistency_surfaces:
        lines.append(
            f"- `{surface.contract_id}` — {surface.mode}/{surface.status}; "
            + '; '.join(surface.notes[:2])
        )
    lines.append('')
    return '\n'.join(lines).strip() + '\n'
